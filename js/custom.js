...
function type() {
    if (charIndex < sentences[index].length) {
        document.getElementById("text").innerHTML += sentences[index].charAt(charIndex);
        charIndex++;
...
function deleteText() {
    if (charIndex > 0) {
        document.getElementById("text").innerHTML = sentences[index].substring(0, charIndex - 1);
        charIndex--;
        setTimeout(deleteText, deletingSpeed);
    } else {
        index = (index + 1) % sentences.length; // go to the next sentence
...
let hasAnimated = false; // Variable to check if animation has occurred

function animateAllSkillBars() {
  skillBars.forEach(skill => {
    const progress = skill.children[0];
    switch(skill.id) {
      case "skill1": progress.style.width = "90%"; break;
      case "skill2": progress.style.width = "90%"; break;
      case "skill3": progress.style.width = "90%"; break;
      case "skill4": progress.style.width = "90%"; break;
      case "skill5": progress.style.width = "90%"; break;
      case "skill6": progress.style.width = "90%"; break;
    }
    skill.classList.add("animated"); // Mark as animated
  });
}

function checkScroll() {
  if (hasAnimated) return; // If already animated, do nothing
  ...
  skillBars.forEach(skill => {
    ...
    if (skillPosition < screenPosition) {
      hasAnimated = true; // Set to true to prevent further animations
      animateAllSkillBars(); // Animate all skill bars
      return; // Break loop after triggering animation
    }
  });
}
